    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            <div className="flex justify-between items-center p-6 border-b border-[#4a5d23]/10 bg-white">
              <h2 className="text-2xl font-serif font-bold text-[#4a5d23]">Reviews: {item.name}</h2>
              <button onClick={onClose} className="p-2 hover:bg-[#4a5d23]/5 rounded-full text-[#4a5d23] transition-colors">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="overflow-y-auto flex-grow p-6">
              {isFormOpen ? (
                <form onSubmit={handleSubmit} className="space-y-6">
                  <h3 className="font-serif text-xl font-bold text-[#4a5d23] mb-4">Write a Review</h3>
                  <div>
                  ...
                  <div className="pt-4 flex gap-4">
                    <button type="button" onClick={() => setIsFormOpen(false)} className="w-full bg-[#f9f7f2] border border-[#c18f58] text-[#c18f58] py-4 rounded-xl font-bold uppercase tracking-widest hover:bg-[#c18f58] hover:text-white transition-all">
                      Cancel
                    </button>
                    <button type="submit" className="w-full bg-[#4a5d23] text-white py-4 rounded-xl font-bold uppercase tracking-widest hover:bg-[#c18f58] transition-all shadow-lg">
                      Submit
                    </button>
                  </div>
                </form>
                ...
                <div className="space-y-6">
                 {reviews.length === 0 ? (
                   ...
                   reviews.map(review => (
                     <div key={review.id} className="bg-white p-6 rounded-2xl shadow-sm border border-[#4a5d23]/5">
                       <div className="flex justify-between items-start mb-3">
                         <div>
                            <span className="font-bold text-[#2a2825]">{review.name}</span>
                            <div className="flex space-x-1 mt-1 text-[#c18f58]">
                              {[...Array(5)].map((_, i) => (
            ...
            {!isFormOpen && (
              <div className="p-6 bg-white border-t border-[#4a5d23]/10">
                <button 
                  onClick={() => setIsFormOpen(true)}
                  className="w-full bg-[#c18f58] text-white py-4 rounded-xl font-bold uppercase tracking-widest hover:bg-[#4a5d23] transition-all flex items-center justify-center space-x-2 shadow-lg"
                >
